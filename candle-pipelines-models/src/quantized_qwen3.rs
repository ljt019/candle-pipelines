//! Patched Qwen3 quantized implementation with external KV cache.
//!
...
//!
//! Based on candle-transformers quantized_qwen3.
...
/// External KV cache for Qwen3 - one per conversation.
#[derive(Debug, Clone)]
pub struct Cache {
...
impl ModelWeights {
    ...
    pub fn forward(&self, input: &Tensor, cache: &mut Cache) -> Result<Tensor> {
        ...
        for (layer_idx, layer) in self.layers.iter().enumerate() {
            h = layer.forward(&h, causal_mask.as_ref(), offset, &mut cache.kvs[layer_idx])?;
        }
