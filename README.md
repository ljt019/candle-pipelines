...
Use `run_iter` to receive tokens as they're generated. Streaming is fully sync - no async runtime needed.

Instead of returning the completion this method returns an iterator you can iterate on to receive tokens individually as they are generated by the model instead of just receiving them all at once at the end.
...
fn main() -> Result<()> {
    ...
    let mut stream = pipeline.run_iter(
...
fn main() -> Result<()> {
    ...
    let stream = pipeline.run_iter("Explain your reasoning step by step.")?;
